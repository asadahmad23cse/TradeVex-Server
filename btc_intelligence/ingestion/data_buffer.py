...
class MarketDataBuffer:
    def __init__(
        ...
        self.force_orders: deque[dict[str, Any]] = deque(maxlen=1500)
        self.depth: dict[str, list[list[float]]] = {'bids': [], 'asks': []}

        self.multi_exchange: dict[str, dict[str, Any]] = {
            'bybit': {'trades': deque(maxlen=trades_max), 'depth': {'bids': [], 'asks': []}, 'cvd': deque(maxlen=500)},
            'okx': {'trades': deque(maxlen=trades_max), 'depth': {'bids': [], 'asks': []}, 'cvd': deque(maxlen=500)},
        }
        ...
        self.open_interest_hist: deque[dict[str, float]] = deque(maxlen=800)
        self.funding_hist: deque[dict[str, float]] = deque(maxlen=300)
    ...
    async def set_multi_exchange(self, exchange: str, trades: list[dict[str, Any]], depth: dict[str, list[list[float]]], cvd_value: float) -> None:
        async with self.lock:
    ...
    async def set_rest_metrics(self, funding_rate: float, mark_price: float, open_interest: float, ticker_24h: dict[str, Any]) -> None:
        async with self.lock:
            ...
            self.open_interest_hist.append({'open_interest': open_interest, 'ts': ts})
            self.funding_hist.append({'funding_rate': funding_rate, 'ts': ts})
